...
NODE_COMMIT="7658ffaaa38d2638ab3cc4c54d11349fd93c4682"
...
LIBCHROMIUMCONTENT_COMMIT="bdb1c8e9d2f184ebeb75a25824aca0be27aa879b"
...
SLOT="$(ver_cut 1-2)"
KEYWORDS="~amd64"
IUSE="cups custom-cflags gconf gnome-keyring jumbo-build kerberos lto neon pic
...
src_prepare() {
	...
	mkdir -p "${CHROMIUM_S}/libchromiumcontent" || die
	cp -a "${LIBCC_S}" "${CHROMIUM_S}/" || die
	...
	local keeplibs=(
		...
		third_party/ced
		third_party/cld_3
		third_party/content_shell_fonts
		third_party/crc32c
...
src_configure() {
	...
	if ! use custom-cflags; then
		...
		if [[ ${myarch} == amd64 || ${myarch} == x86 ]]; then
			filter-flags -mno-mmx -mno-sse2 -mno-ssse3 \
				-mno-sse4.1 -mno-avx -mno-avx2
		fi
	fi
	...
	gn_target="//libchromiumcontent/chromiumcontent:chromiumcontent"
	myconf_gn+=" root_extra_deps = [\"${gn_target}\"]"
...
src_compile() {
	...
	eninja -C "${chromium_target}" \
		libchromiumcontent/chromiumcontent:chromiumcontent
	CHROMIUMCONTENT_2ND_PASS=1 \
		eninja -C "${chromium_target}" \
			libchromiumcontent/chromiumcontent:libs
...
src_install() {
	...
	doins out/R/v8_context_snapshot.bin
